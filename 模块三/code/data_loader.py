...
def load_node_price() -> pd.DataFrame:
    ...
    读取 config.PRICE_XLSX，取「节点实时电价_元MWh」列，
    除以 1000 转为元/kWh。文件缺失时回退到 mock 占位曲线。
    """
    import config as C

    if os.path.exists(C.PRICE_XLSX):
        df = pd.read_excel(C.PRICE_XLSX)
        rt_col = "节点实时电价_元MWh"
        if rt_col not in df.columns:
            rt_col = df.columns[-1]
        ts = pd.to_datetime(df["日期"]) + pd.to_timedelta(df["小时"], unit="h")
        price_kwh = df[rt_col].astype(float) / 1000.0  # 元/MWh → 元/kWh
        out = pd.DataFrame({"timestamp": ts, "price": price_kwh.values})
        out = out.dropna(subset=["price"]).sort_values("timestamp").reset_index(drop=True)
...
def build_schedule_from_module2() -> pd.DataFrame:
    ...
    json_path = os.path.join(C.PROJECT_DIR, "模块二", "智能调度", "output", "yearly_dispatch.json")
