...
def daily_opt_dispatch(
    ...
    c = np.zeros(nvar)
    for t in range(H):
        c[i_pgrid + t] = w_cost * price[t] + w_green * 1.0
    ...
    load_total = flex.sum()          # 实际用电总量（柔性负荷可调后）
    green_kwh = load_total - net_grid_kwh
    green_ratio = green_kwh / load_total * 100 if load_total > 0 else 0.0
