...
def capacity_planning():
    ...
    print("\n" + "=" * 104)
    print("方案对比表（两目标：分时成本 + 绿电）")
    print("=" * 104)
    header = (f"{'光伏MW':>6} {'储能kWh':>8} {'储能kW':>7} {'投资万':>7} "
              f"{'年节省万':>8} {'回收期':>7} {'绿电%':>6} {'上网%':>6} {'无解':>4}")
    print(header)
    print("-" * 104)

    results = []
    for pv_scale in pv_scales:
        # pv_arr_base 已含 PV_SCALE(2.0) 放大，这里按"相对 6.08MW 基准的倍率"还原再放大，
        # 修正二次放大 bug（此前 pv_arr_base * pv_scale 把光伏翻倍成 24.3MW，虚高绿电到 44%）
        ...
        for E_max, P_max in batteries:
            ...
            annual_saving = base_cost - run_cost
            payback = total_invest / annual_saving if annual_saving > 0 else float("inf")
            green = (ops["load"] - ops["net"]) / ops["load"] * 100
            ...
            print(f"{eco['pv_mw']:>6.1f} {E_max:>8.0f} {P_max:>7.0f} {eco['invest_wan']:>7.0f} "
                  f"{eco['saving_wan']:>8.0f} {pb:>7} {eco['green']:>6.1f} "
                  f"{eco['export_pct']:>6.1f} {eco['fail']:>4}")
